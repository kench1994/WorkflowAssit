...
public:
    ...
    void send(const char* bin_data)
    {
		zstr_sendf(zmq_sock_ptr_, "%s", bin_data);
        fprintf(stdout, "client<%d> send_str:%s\n", client_id_, bin_data);
    }
	
	void test_send() {
        fprintf(stdout, "client<%d> send data\n", client_id_);
	...
	int do_recv() {
		...
		*/
		...
		if (items[0].revents & ZMQ_POLLIN) {
            ...
            fprintf(stdout, "client<%d> recv_str:%s\n", client_id_, zframe_data(content));
